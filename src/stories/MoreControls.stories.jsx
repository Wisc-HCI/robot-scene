  ...
  onMove: (id, source, worldTransform, localTransform) =>
    set((state) => {
      if (
        ...
        const value = clamp(
          (10 * localTransform.position.z + 0.5) *
        ...
        if (id === "scalarInputIndicator") {
          ...
          // Update visuals
          state.items.topRangeInputIndicator.position.z =
            ((state.rangeValue[1] - state.range[0]) /
              (state.range[1] - state.range[0])) *
              0.1 -
            0.05;
          state.items.bottomRangeInputIndicator.position.z =
            ((state.rangeValue[0] - state.range[0]) /
              (state.range[1] - state.range[0])) *
              0.1 -
            0.05;
          ...
          state.items.rangeInputRange.shapeParams.height =
            ((state.rangeValue[1] - state.rangeValue[0]) /
              (state.range[1] - state.range[0])) *
            0.09;
          // Update the text above
          state.texts.inputRangeText.value = `${
            Math.round(state.rangeValue[0] * 100) / 100
          }, ${Math.round(state.rangeValue[1] * 100) / 100}`;
        }
      } else {
...
const Template = (args) => {
  ...
  const [play, pause, reset] = useStore((state) => [
    state.play,
    state.pause,
    state.reset,
  ]);
  ...
  const [scalarValue, setScalarValue] = useStore(
    (state) => [state.scalarValue, state.setScalarValue],
    shallow
  );
  const [rangeValue, setRangeValue] = useStore(
    (state) => [state.rangeValue, state.setRangeValue],
    shallow
  );
...
const getScalarInputItems = (rangeBounds, value, offset) => {
  const z =
    ((value - rangeBounds[0]) / (rangeBounds[1] - rangeBounds[0])) * 0.1 - 0.05;
  return {
    scalarInputHousing: {
      shape: "capsule",
      name: "Input Housing",
...
const getRangeInputItems = (rangeBounds, valueRange, offset) => {
  const zSphereTop =
    ((valueRange[1] - rangeBounds[0]) / (rangeBounds[1] - rangeBounds[0])) *
      0.1 -
    0.05;
  const zSphereBottom =
    ((valueRange[0] - rangeBounds[0]) / (rangeBounds[1] - rangeBounds[0])) *
      0.1 -
    0.05;
  const rangeHeight =
    ((valueRange[1] - valueRange[0]) / (rangeBounds[1] - rangeBounds[0])) *
    0.09;
